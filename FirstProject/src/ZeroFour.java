public class ZeroFour {
    public static void main(String[] args) {
        System.out.println(10);
        System.out.println(10-10);
        System.out.println(10- -10);
        System.out.println(5+2);
        System.out.println(5+2*5);
        System.out.println((5+2)*5);
        System.out.println(4-3-3);
        System.out.println(4-(3-3));
        System.out.println(-4 - -4 - -4);
        System.out.println(6/2);
        System.out.println(6/3);
        System.out.println(6/4);
        System.out.println(6%2);
        System.out.println(6%3);
        System.out.println(6%4);
        System.out.println(-6%4);
        System.out.println(6%-4);
        System.out.println(-6%-4);

        System.out.println(5 + 2.0);
        System.out.println(5.0 + 2);
        System.out.println((7/4)+5);
        System.out.println((7/4.0)*5);
        System.out.println((int)5);
        System.out.println((int)5.3);
        System.out.println((int) 5.9);
        System.out.println((int) -5.3);
        System.out.println((7/(double)4));
        System.out.println((double)7/4);
        System.out.println((double)(7/4));
        System.out.println((int)(double)(7/4));
        System.out.println((int)(double)4);
        System.out.println((int)(double)4.3);
        System.out.println((double)(int)4.3);
        System.out.println((int)-5.9);
        System.out.println(2*(7%4));
        System.out.println(2*(double)(7%4));
        System.out.println(2*((double)7%4));
        System.out.println(2*(7%(double)4));





    }
}
